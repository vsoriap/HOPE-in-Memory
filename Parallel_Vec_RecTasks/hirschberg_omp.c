...
hirschberg(const char *a, const char *b)
{
    ...
    if (m > n) {
        ...
        #pragma omp parallel 
        {
            ...
            {
                #pragma omp task
                hirschberg_recursive(c, b, n, a, m, 1, 0);
            }
        }
    ...
    {   
        ...
        {   
            #pragma omp task
            hirschberg_recursive(c, a, m, b, n, 1, 0);
        }
    }
    return c;
}
...
hirschberg_recursive(char *c, const char *a, unsigned int m, const char *b, unsigned int n, char right, int depth){
    if (n > 1) {
        ...
        if (depth < 3){
            ...
            #pragma omp parallel if(m >= 10000) num_threads(1)
            {
...
nwlcost(int *s, const char *a, unsigned int m, const char *b, unsigned int n)
{
    ...
    #pragma omp parallel private(i,j,l,p,q) firstprivate(act,prev,pprev,k) if(m >= 10000) num_threads(2)
    {
        ...
        while(i <= n && j <= m) {
            ...
            if (q == 0) { // Do diagonals until reaching the last row
                ...
                if (maxII < 2048){
                    ...
                    starti = ithread*maxII/nthreads;
                    finishi = (ithread+1)*maxII/nthreads;
                    p_i = p - starti;
                    q_i = q + starti;
                    iactive = 1;
                    #pragma omp barrier
                ...
                if(ithread == 0){        // Compute first cell that corresponds with first column
                    act[0] = prev[0] + 1;
                    p_i--; q_i++; l=1;
                    starti = 1;
                }
                ...
                if (iactive == 1){
                    ...
                    for(l = starti; l < finishi; l++){
                        int cost[3], tmp;
                        cost[0] = prev[l] + 1;
                        cost[1] = pprev[l] + (a[q_i-1] != b[p_i-1]);
                        cost[2] = prev[l+1] + 1;
                        tmp = cost[0] < cost[1] ? cost[0] : cost[1];
                        tmp = tmp < cost[2] ? tmp : cost[2];
                        act[l] = tmp;
